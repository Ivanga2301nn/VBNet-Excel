...
"
    ...
    Public Sub PV_New_zapiska()
        ...
        Try
            ...
            Външно_Захранване(wordDoc, dicSignature)
            Пожарна_БЕЗ_ПИЦ(wordDoc, dicSignature)
            Заземителна(wordDoc, dicSignature)
            Мълниезащита(wordDoc, acDoc, acCurDb)
            Заключение(wordDoc, dicSignature)
        ...
        End Try
    End Sub
    Sub Пожарна_БЕЗ_ПИЦ(wordDoc As Word.Document, dicSignature As Dictionary(Of String, String))
        Dim Text As String = ""
        FormatParagraph(wordDoc, "ПОЖАРНА БЕЗОПАСНОСТ", wordApp, level:=1, resetLevel:=False)





        ' --- СЕКЦИЯ: СИСТЕМА ЗА КОМПЛЕКСНА БЕЗОПАСНОСТ, ПОЖАРОГАСЕНЕ И ТЕРМОМЕНИДЖМЪНТ ---

        ' Основно заглавие на раздела
        FormatParagraph(wordDoc, "ПОЖАРОГАСЕНЕ И ТЕРМОМЕНИДЖМЪНТ НА ЛСС", wordApp, 2, True)
        ' Въведение
        Text = "Локалните съоръжения за съхранение на електрическа енергия (ЛСС) представляват високотехнологични модулни системи, проектирани за работа в условия на интензивни цикли на заряд и разряд."
        Text += "Предвид химическата специфика на литиево-йонните батерии, основният риск при тях е възникването на thermal runaway (неконтролируемо термично саморазвитие), което води до отделяне на запалими газове, локално възпламеняване и опасност от вторично запалване."
        AddParagraph(wordDoc, Text, False)
        Text = "За намаляване на този риск, противопожарната система е разработена като многоетапен защитен модел."
        Text += "Тази многослойна архитектура обхваща превенция, ранна детекция, контрол на енергийния процес, локализация на събитието и последващо охлаждане, интегрирайки конструктивна защита, активна пожарна автоматика и интелигентен термален контрол."
        AddParagraph(wordDoc, Text, False)
        Text = "ЛСС са специално проектирани за безопасно съхранение и експлоатация на литиево-йонни батерии. "
        Text += "Те са оборудвани със система за пожарогасене, която осигурява ранно откриване и ефективна реакция при възникване на пожар. "
        Text += "Оборудването включва аерозолно пожарогасително устройство, димни и температурни детектори, "
        Text += "звукови и визуални аларми, както и механизъм за автоматично прекъсване на захранването."
        AddParagraph(wordDoc, Text, False)
        Text = "Аерозолната система може да се задейства самостоятелно, като веднага след активирането ѝ електроподаването се преустановява."
        AddParagraph(wordDoc, Text, False)
        Text = "Конфигурацията на системата е проектирана така, че да осигурява многостепенна защита при възникване на "
        Text += "аварийни режими, с цел ограничаване на риска от възникване и развитие на пожар в резултат на термични или електрически откази в батерийните модули."
        AddParagraph(wordDoc, Text, False)
        ' --- 1. Конструктивни мерки ---
        FormatParagraph(wordDoc, "Конструктивни мерки и физическа защита", wordApp, 3)
        Text = "Корпусът на всяко ЛСС е проектиран като пожароустойчив енергиен контейнер (тип Energy Storage Container), осигуряващ:"
        AddParagraph(wordDoc, Text, False)
        AddParagraph(wordDoc, "• Огнеустойчивост: Ограждащите елементи(стени и врати) предотвратяват разпространението на огън към съседни съоръжения.", False, 35.4375, True)
        AddParagraph(wordDoc, "• Степен на защита: Висока IP защита от външни въздействия (прах, влага) с интегрирана антикорозионна обработка", False, 35.4375, True)
        AddParagraph(wordDoc, "• Взривозащита: Наличие на предпазни клапани (venting panels) за освобождаване на свръхналягане.", False, 35.4375, True)
        ' --- 2. Автоматизирана система за пожарогасене ---
        FormatParagraph(wordDoc, "Автоматизирана система за пожарогасене (АСПГ)", wordApp, 3)
        Text = "За осигуряване на максимална безопасност е заложена специализирана АСПГ с цифрово управление."
        Text += " Техническото решение включва аерозолни генератори с диелектричен гасителен агент, интелигентни димно-оптични сензори и топлинни датчици."
        Text += "АСПГ разполага със собствено резервирано захранване (UPS), гарантиращо работоспособност на системата при пълна авария."
        AddParagraph(wordDoc, Text, False)
        Text = "Защитата се базира на специализирана АСПГ с автономно цифрово управление и диелектричен аерозолен гасителен агент."
        Text += " Системата работи по тристепенен алгоритъм."
        AddParagraph(wordDoc, Text, False)
        ' --- 3. Алгоритъм на защитната реакция ---
        AddParagraph(wordDoc, "Алгоритъм на защитната реакция и превенция", True)
        AddParagraph(wordDoc, "• Превантивно ниво (BMS Контрол): Системата за управление на батериите следи напрежението и температурата на всяка клетка, като автоматично ограничава мощността при аномалия.", False, 35.4375, True)
        Text = "• Детекция и алармена сигнализация: При засичане на дим или критична промяна в температурата "
        Text += "се активират звукови и светлинни сигнали за ранно предупреждение. Температурният детектор "
        Text += "следи в реално време за отклонения и задейства пожарна аларма при достигане на рискови стойности."
        AddParagraph(wordDoc, Text, False, 35.4375, True)
        Text = "В първично ниво системата за управление на батериите (BMS) извършва непрекъснат контрол върху електрически и температурни параметри на всяка клетка."
        Text += "При отклонение от допустимите стойности се активират ограничителни режими или изключване на съответния модул."
        AddParagraph(wordDoc, Text, False, 35.4375, True)
        Text = "• Аерозолно гасене: Ако температурата на клетките достигне ≥ 47°C или бъде "
        Text += "засечен дим, аерозолното устройство се активира автоматично за локализиране и потушаване на пожара. "
        Text += "Процесът на гасене се основава на прекъсване на химическата реакция на горене."
        Text += "Системата предоставя обратна връзка за състоянието на гасенето в реално време."
        AddParagraph(wordDoc, Text, False, 35.4375, True)
        Text = "• Автоматично изключване на захранването: При регистриране на пожар или задействане на "
        Text += "гасителното устройство, защитният прекъсвач незабавно изключва захранването."
        AddParagraph(wordDoc, Text, False, 35.4375, True)
        ' --- 4. Термален мениджмънт ---
        Text = "Система за прецизен термален мениджмънт"
        AddParagraph(wordDoc, Text, True)
        Text = "Системата поддържа температурата в тесен работен диапазон чрез интелигентна циркулация на флуид."
        AddParagraph(wordDoc, Text, False)
        ' --- Секция Охлаждане ---
        Text = "Охлаждане и защита на батериите: ЛСС разполага с електрически шкаф с блок за течно охлаждане. "
        Text += "Тази система поддържа оптимален топлинен режим, което гарантира безопасна работа."
        AddParagraph(wordDoc, Text, False)
    End Sub
    '
    ...
    Sub ЗАПИСКА_Батерия(wordDoc As Word.Document,
        ...
        With Text
            Text = $"В проекта е предвидено да се {If(dicObekt("intBattery_Count") > 1, "монтират", "монтира")} "
            Text += $"{dicObekt("intBattery_Count")} {If(dicObekt("intBattery_Count") > 1, "броя локални съоръжения", "брой локално съоръжение")} "
            Text += $"за съхранение на електрическа енергия тип {dicObekt("strBattery_Name")}, "
            Text += $"{If(dicObekt("intBattery_Count") > 1, "изградени", "изградено")} от батерийни клетки {dicObekt("strBattery_Type")}."
            AddParagraph(wordDoc, Text, False)
...
#End Region
    ...
    Sub ОБЯСНИТЕЛНА_ЗАПИСКА_PV(wordDoc As Word.Document,
        ...
        If dicObekt("intBattery_Count") > 0 Then
            Text = $"В проекта е предвидено да се монтират {dicObekt("intBattery_Count")} {If(dicObekt("intBattery_Count") > 1, "броя локални съоръжения", "брой локално съоръжения")} за съхранение на електрическа енергия тип {dicObekt("strBattery_Name")} изградени от батерийни клетки и от батерийни клетки {dicObekt("strBattery_Type")}."
        ...
        With Text
            ...
            txtZemya += " Местоположението на устройството за ръчно прекъсване на веригите е изрично указано в графичната част на проекта."
            txtZemya += vbCrLf
            ...
            txtPokriv += "В проекта не е предвидено разполагане на фотоволтаични модули върху брандмауери и над вертикални пожарозащитни прегради."
            txtPokriv += " Предвидено е минимално отстояние до такива прегради не по-малко от 1,0 m." + vbCrLf
            txtPokriv += "Не е предвиден монтаж на фотоволтаични модули в огнеустойчиви фасадни участъци, както и на разстояние по-малко от 1,0 m от евакуационни изходи, с оглед осигуряване на необходимите изисквания за пожарна безопасност." + vbCrLf
            ...
            Else
                ...
                txtPokriv += "Монтажната конструкция за закрепване на фотоволтаичните модули е изпълнена от материали с клас по реакция на огън не по-нисък от А2." + vbCrLf
            ...
            End If
            Dim txtKabeli As String = ""
            txtKabeli += "Кабелните трасета за постоянен ток, преминаващи през сградата, са проектирани при повишени изисквания за пожарна безопасност."
            txtKabeli += " Кабелите са положени в защитни тръби или канали с минимален клас по реакция на огън А2."
            txtKabeli += " Като алтернатива са предвидени защитни строителни продукти, осигуряващи минимална огнеустойчивост EI 30 по цялата дължина на преминаване през конструктивните елементи и помещенията на сградата." + vbCrLf
            txtKabeli += "В непосредствена близост до входа на сградата е предвидено табло с документация, съдържаща информация за местоположението на фотоволтаичните модули, инверторите, прекъсващите устройства и други основни елементи на системата." + vbCrLf
            txtKabeli += "До всеки вход на сградата е предвидено поставяне на предупредителен знак, указващ наличието на фотоволтаична електрическа централа, съгласно изискванията за пожарна безопасност." + vbCrLf
            Select Case dicObekt("Монтаж")
                Case "Земя"
...
'Съгласно НАРЕДБА № Iз-1971 ОТ 29 ОКТОМВРИ 2009 Г. ЗА СТРОИТЕЛНО-ТЕХНИЧЕСКИ ПРАВИЛА И НОРМИ ЗА ОСИГУРЯВАНЕ НА БЕЗОПАСНОСТ ПРИ ПОЖАР в сила от 05.06.2010 г., Издадена от министъра на вътрешните работи и министъра на регионалното развитие и благоустройството , Обн. ДВ. бр.96 от 4 Декември 2009г., попр. ДВ. бр.17 от 2 Март 2010г., изм. ДВ. бр.101 от 28 Декември 2010г., изм. и доп. ДВ. бр.75 от 27 Август 2013г., изм. и доп. ДВ. бр.69 от 19 Август 2014г., изм. и доп. ДВ. бр.89 от 28 Октомври 2014г., изм. ДВ. бр.8 от 30 Януари 2015г., изм. и доп. ДВ. бр.2 от 8 Януари 2016г., изм. и доп. ДВ. бр.1 от 3 Януари 2017г., изм. и доп. ДВ. бр.63 от 31 Юли 2018г., изм. и доп. ДВ. бр.91 от 29 Октомври 2024г.,
'Изискванията към фотоволтаичната инсталация са:
'Чл. 350а. (Нов - ДВ, бр. 91 от 2024 г., в сила от 31.12.2024 г.) (1) Фотоволтаичните електрически централи за производство на електрическа енергия от слънчева енергия, предвидени за монтаж към сгради и постройки, в т.ч. върху покривните и фасадните им конструкции и в прилежащите им поземлени имоти, се изграждат при спазване на следните изисквания:
