...
"
    ...
    Private Sub AddParagraph(ByVal wordDoc As Word.Document,
        ...
        ' Дефиниране на променливи за форматиране
        Dim fontName As String = "Cambria"
        Dim fontSize As Integer = 12
        Dim fontBold As Boolean = Bold
        Dim paragraphAlignment As Word.WdParagraphAlignment = Word.WdParagraphAlignment.wdAlignParagraphJustify
        Dim spaceBefore As Single = 0
        Dim spaceAfter As Single = 0
        Dim lineSpacing As Word.WdLineSpacing = Word.WdLineSpacing.wdLineSpaceSingle
        Dim leftIndent As Single = 0
        Dim rightIndent As Single = 0
        Dim firstLineIndent As Single = FirstLine

        ' Добавяне на параграф
    ...
    Sub ОБЯСНИТЕЛНА_ЗАПИСКА_PV(wordDoc As Word.Document,
        ...
        With Text
            ...
            With wsFEC_ФЕЦ
                For i = 3 To 51 Step 5
                    ...
                    Text += IIf(.Cells(5, i).Value > 1, String.Format("{0:0}", .Cells(5, i).Value), "")
                    Text += IIf(.Cells(5, i).Value > 1, "бр. инвертори", "инвертор")
                    Text += " тип "
                    Text += .Cells(25, i).Value.ToString
                    Text += IIf(.Cells(5, i).Value > 1,
                                ", с номера ",
                                ", с номер ")
                    ' Проверка дали клетката съдържа нещо, което не е число
                    If Not IsNumeric(.Cells(24, i).Value) Then
                        ' Ако стойността не е число, прекратяваме цикъла
                        Text += .Cells(24, i).Value
                    Else
                        Text += .Cells(24, i).Value.ToString
                    End If
                    Text += IIf(.Cells(5, i).Value > 1,
                                ". На всеки инвертор в групата са монтирани ",
                                ". На инвертора са монтирани ")
                    Text += String.Format("{0:0}", .Cells(35, i).Value)
                    Text += " бр. панели тип "
                    Text += .Cells(7, i).Value
                    Text += IIf(.Cells(5, i).Value > 1,
                                ". Общата мощност на панелите за всеки инвертор е ",
                                ". Общата мощност на панелите за инвертора e ")
                    Text += String.Format("{0:0.000}", .Cells(36, i).Value / 1000)
                    Text += " kWp."

                    If .Cells(5, i).Value > 1 Then
                        Text += " Общата генерирана, от панелите, за група "
                        Text += String.Format("{0:0}", br_Grupi)
                        Text += " мощност е "
                        Text += String.Format("{0:0.000}", .Cells(34, i).Value)
                        Text += " kWp."
                    End If
                    Text += " Оптималното напрежение за MPPT е "
                    Text += String.Format("{0:0}", .Cells(27, i).Value)
                    ...
                    Text += String.Format("{0:0}", .Cells(28, i + 3).Value)
                    Text += " V."
        ...
        FormatParagraph(wordDoc, "МОНТАЖ НА СОЛАРНИ ПАНЕЛИ, СОЛАРНИ КАБЕЛИ И МРЕЖОВИ МНОГОСТРИНГОВИ ИНВЕРТОРИ", wordApp, level:=2)
        With Text
            If dicObekt("Монтаж") = "Земя" Then
                If dicObekt("Монтаж") = "Земя" Then

                End If
                Text = "Конструкцията, върху която се монтират фотоволтаичните модули, е с клас по реакция на огън А2."
                Text += " Отстоянието на фотоволтаични модули до сгради, постройки и съоръжения е не по-малко от 4 m."
                Text += " В зоната на монтаж на соларните панели няма надземни газопроводи за ГГ и надземни тръбопроводи за ЛЗТ и ГТ."
                AddParagraph(wordDoc, Text, False)
                Text = " До входа на площадката на фотоволтаичната централи е осигуряват пътища за противопожарни цели при спазване изискванията на чл. 27, ал. 3, 7, 8 и 9 от НАРЕДБА № Iз-1971 от 29 октомври 2009 г. за строително-технически правила и норми за осигуряване на безопасност при пожар"
                AddParagraph(wordDoc, Text, False)
                Text = "В близост до входа на фотоволтаичната централа в предвидено устройство за ръчно прекъсване на веригите за постоянен ток и за променлив ток на фотоволтаичната електрическа централа."
                Text += "Конкретния начин за реализация на това изключване е отразено в графичната част на проекта."
                Text += " Местоположението на устройството за ръчно прекъсване на веригите е изрично указано."
                AddParagraph(wordDoc, Text, False)
                Text = " След въвеждане в експолатация в специално табло да се постави документация с информация за фотоволтаичната електрическа централа, определена в Наредба № 8121з-647 от 2014 г. за правилата и нормите за пожарна безопасност при експлоатация на обектите."
                AddParagraph(wordDoc, Text, False)
            Else
                Text = $"Чл. 350а. (Нов - ДВ, бр. 91 от 2024 г., в сила от 31.12.2024 г.) (1) Фотоволтаичните електрически централи за производство на електрическа енергия от слънчева енергия, предвидени за монтаж към сгради и постройки, в т.ч. върху покривните и фасадните им конструкции и в прилежащите им поземлени имоти, се изграждат при спазване на следните изисквания
...
бб) хидроизолацията на покрива е с клас по реакция на огън, по-нисък от А2, или с неустановени експлоатационни показатели по отношение на реакцията и на огън и е положена върху повърхности с клас по реакция на огън А1 или А2;
...
в) разстоянието между фигурите по буква {Кавички}б{Кавички} е не по-малко от 1,5 m;
...
б) фотоволтаичните модули се разполагат на фигури с размери не по-големи от 40 m х 40 m;
в) разстоянието между фигурите по буква {Кавички}б{Кавички}, както и разстоянието от същите фигури до сградите, постройките и съоръженията (с изключение на съоръженията от категория по пожарна опасност Ф5Д) в поземления имот и в съседните поземлени имоти, е не по-малко от 1,5 m;
