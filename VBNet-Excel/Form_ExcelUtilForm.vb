...
Public Class Form_ExcelUtilForm
    ...
    End Sub
    ''' <summary>
    ''' Номерира, форматира и финализира количествената сметка в работния лист wsKol_Smetka.
    '''
    ''' Процедурата изпълнява цялостна автоматична обработка на листа:
    '''
    ''' 1) НОМЕРИРАНЕ НА РЕДОВЕТЕ
    '''    - Започва от ред 6.
    '''    - Номерира само редове, които:
    '''         • не са оцветени с Accent6
    '''         • не започват с " - "
    '''    - Прескача разделителни и оцветени редове.
    '''
    ''' 2) АВТОМАТИЧНА ВИСОЧИНА НА РЕДОВЕТЕ
    '''    - Изчислява височината според дължината на текста в колона B.
    '''    - Умножава по базова височина:
    '''         • 15.75 при натиснат "Номерирай"
    '''         • 15 при натиснат "Свий лист"
    '''
    ''' 3) ОБЩО ФОРМАТИРАНЕ НА ЛИСТА
    '''    - Задава шрифт Cambria 12
    '''    - Центрира вертикално и хоризонтално
    '''    - Настройва ширини на колоните
    '''    - Активира WrapText
    '''
    ''' 4) ФОРМАТИРАНЕ НА ЗАГЛАВНАТА ЧАСТ
    '''    - Обединява клетки A1:D1 и A2:D2
    '''    - Прилага различно подравняване и удебеляване
    '''    - Динамично изчислява височината на ред 2
    '''
    ''' 5) ОФОРМЯНЕ НА РАЗДЕЛИТЕЛ "ЧАСТ ЕЛЕКТРО"
    '''    - Центриране
    '''    - Фоново оцветяване Accent4
    '''    - Използва Excel_Kol_smetka_Razdel()
    '''
    ''' 6) ПОДРАВНЯВАНЕ ПО КОЛОНИ
    '''    - A → център
    '''    - B → ляво
    '''    - C → център
    '''    - D → дясно + отстъп
    '''
    ''' 7) ПРИЛАГАНЕ НА РАМКИ
    '''    - Външна рамка Medium
    '''    - Вътрешни линии (Horizontal Dot / Vertical Thin)
    '''
    ''' 8) ДОБАВЯНЕ НА ЗАКОНОВ ТЕКСТ И ЗАБЕЛЕЖКА
    '''    - Автоматично сливане на клетки
    '''    - Динамична височина
    '''    - Частично Bold и Underline
    '''
    ''' 9) ДОБАВЯНЕ НА ПОДПИС НА ПРОЕКТАНТА
    '''    - Центриране Across Selection
    '''    - Попълване от ComboBox_Proektant
    '''
    ''' 10) ФИНАЛНА ОБРАБОТКА НА РАЗДЕЛИТЕЛНИ РЕДОВЕ
    '''     - Прилага Excel_Kol_smetka_Razdel() според оцветяване
    '''
    ''' 11) Актуализира ProgressBar по време на изпълнение.
    '''
    ''' Това е завършваща процедура, която превръща суровите данни
    ''' в напълно форматиран и готов за печат документ.
    ''' </summary>
