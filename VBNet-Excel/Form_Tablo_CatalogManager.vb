...
Public Class CableCatalog
    ...
    Public Sub CalculateCable(ByRef tokow As clsTokow,
                          Optional Cable_Type As String = Nothing,    ' Тип кабел ("СВТ", "САВТ", "NYY"...). Nothing = използвай стойността от tokow.
                          Optional layMethod As String = Nothing,     ' Полагане: 0=въздух (35°C), 1=земя (15°C). Nothing = използвай tokow.
        ...
        Dim currentLayMethod As String
        ...
        ' ============================================================
        If Cable_Type IsNot Nothing Then currentType = Cable_Type
        If String.IsNullOrWhiteSpace(currentType) Then currentType = tokow.Кабел_Тип
        If String.IsNullOrWhiteSpace(currentType) Then currentType = "СВТ" ' Аварийна стойност при липсващи данни
        ' ============================================================
        ...
        Else
            If cableInfo IsNot Nothing Then currentMaterial = cableInfo.Material
        End If
        If String.IsNullOrWhiteSpace(currentMaterial) Then currentMaterial = "Cu" ' Аварийна стойност при липсващи данни
        ' ============================================================
        ...
        ' 3. Стойност по подразбиране
        ' ============================================================
        If layMethod IsNot Nothing Then currentLayMethod = layMethod
        If String.IsNullOrWhiteSpace(currentLayMethod) Then currentLayMethod = tokow.Кабел_Полагане
        If String.IsNullOrWhiteSpace(currentLayMethod) Then currentLayMethod = "Във въздух"


        ' Изчисляваме Inom като вземаме по-голямото число между двете
        Dim Inom As Double = Math.Max(Val(tokow.Breaker_Номинален_Ток), tokow.Ток)
        ...
        Dim Qok As Double = If(currentLayMethod.ToLower.Contains("зем"), 15, 35)
        ' 15°C - директно полагане в земя
        ' 35°C - въздух, тръби, канали и други случаи без директен контакт със земята
        ...
        For i As Integer = 0 To filteredCables.Count - 1
            ...
            Dim Imax As Double = If(currentLayMethod.ToLower.Contains("зем"), cable.MaxCurrent_Ground, cable.MaxCurrent_Air)
            If Imax >= Idop Then
                calc = cable.PhaseSize
                Exit For
            End If
        Next
        ' 4. ИЗВЛИЧАНЕ НА ТОКОВЕ ЗА ГОЛЕМИ СЕЧЕНИЯ (за паралелни жила)
        ...
        tokow.Кабел_Полагане = currentLayMethod
...
Public Class RCDCatalog
    ...
    End Function
    ''' <summary>
    ''' Изчислява и избира подходяща ДТЗ (RCD) за токовия кръг според характеристиките
    ''' на групата, към която принадлежи.
    '''
    ''' Процедурата анализира всички консуматори със същата RCD_Нула, изчислява общия ток,
    ''' определя необходимия брой полюси и избира подходящ апарат от каталога.
    '''
    ''' При успешно намиране на апарат автоматично попълва параметрите на ДТЗ
    ''' в обекта на токовия кръг.
    '''
    ''' Ако не бъде намерен подходящ апарат, извежда предупредително съобщение
    ''' с необходимите параметри за търсене.
    ''' </summary>
    ''' <param name="tokow">
    ''' Токовият кръг, за който се извършва избор и настройка на ДТЗ.
    ''' </param>
    Public Sub SetRCD(tokow As clsTokow)
        ' 1. Защитни проверки за специалните редове
        If tokow Is Nothing OrElse String.IsNullOrWhiteSpace(tokow.RCD_Нула) Then Return
        If tokow.ТоковКръг = "ОБЩО" OrElse
           tokow.ТоковКръг = "Разединител" Then Return

        Dim totalCurrent As Double = 0.0
        Dim hasThreePhase As Boolean = False
        ' 2. Намираме всички консуматори в същата сграда, табло и със същото RCD_Нула
        Dim groupConsumers = AppSettings.ListTokow.Where(Function(t) t.BuildingName = tokow.BuildingName AndAlso
                                                                 t.Tablo = tokow.Tablo AndAlso
                                                                 t.RCD_Нула = tokow.RCD_Нула
                                                                 ).ToList()

        ' 3. Изчисляваме общия ток и проверяваме за трифазни кръгове в групата
        For Each consumer In groupConsumers
            totalCurrent += consumer.Ток
            If consumer.Брой_Полюси = 3 Then
                hasThreePhase = True
            End If
        Next
        ' 4. Определяне на крайната стойност за полюси ("4p" или "2p")
        Dim poles As String = If(hasThreePhase, "4p", "2p")
        ' 5. Твоята формула: 1.2 * общия ток на групата, но не по-малко от 20 A
        Dim requiredCurrent As Double = If(totalCurrent * 1.2 < 20, 20, totalCurrent * 1.2)
